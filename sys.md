...
- 增加排课统计性结果，比如表格形式，统计每个班级、教师、课程的排课情况
- 教室管理及考虑教室排课策略，比如教室容量、教室类型、教室位置等。我们已经制作了教室管理页面和相关接口，但是排课算法中没有考虑教室的问题。教室的最关键的就是容纳数量。
- 优化排课算法，增加参数，比如教师排课优先级、课程分布策略、课程冲突检测策略等
- 增加多位教师负责同一课程的排课策略，比如均衡分布，课时条件限制
- 增加年级管理，比如针对年级排课，教师所属年级等分组分类
- 增加移动端课表查看、分享

添加了统计图表组件，目前图的数字都不准确。（待完善）

需要均衡算法。我们目前的简单遍历算法使教师、班级、课程都够基本对应上。下一步想实现一些自动均衡化调整的优化算法。比如说现在较大比例的课程集中在周一，如果我们想每周各天排的课程数比较相近，是不是可以一键做一个日期分布均衡的调整，还要保持没有冲突。这部分可以作为一个独立函数，既可以在排课之后去优化，也能作为一项配置，在排课前就填入并考虑。先深度思考一下给出解决方案，不写代码 （已完成）

现在比较大的问题是均衡算法的代码。优化调整之后还是出现了对同一老师同一个班同一门课：周三的单周课，周四的单周课，和周五的双周课这种安排。应该合并一个单周和双周，只安排两天的课。我们是否可以加一个额外的函数，在优化过后来检测和合并单双周？当然，也需要有冲突检测避免额外的错误。 （已完成）

在课程管理页增加了一个删除全部数据的按钮，现在需要完成按钮对应的功能。与 clearDatabase()的功能相同。 （已完成）

在课程管理页增加一个下载表格模板的按钮，点击按钮时可以下载一个 excel 表格模板，模板放在了 public 目录下 importDataDemo.xls。
